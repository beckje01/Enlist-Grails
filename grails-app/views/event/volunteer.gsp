...
<body>
	<div class="row-fluid">
		...
		<section id="main" class="span9">
			...
			<g:if test="${event.activities.size()>0}">

				<table class="table table-striped">
					<thead>
						<tr>
							<g:sortableColumn property="name" title="${message(code: 'event.name.label', default: 'Activity')}" />
							<th>Slots</th>
					...
					<tbody>
						<g:each in="${event.activities}" var="activity">
							<tr>
								<td><g:link controller="activity" action="show" id="${activity.id}">${fieldValue(bean: activity, field: "title")}</g:link></td>
								<td>${activity.countOfVolunteers} / ${activity.numPeopleNeeded}</td>
								<td><g:formatDate type="datetime" format="SHORT" date="${activity.startDate}" /></td>
								<td><g:formatDate type="datetime" format="SHORT" date="${activity.endDate}" /></td>
								<td>${fieldValue(bean: activity, field: "location")}</td>
								<td class="link">
